...
class PocketMoneyTxTypeEnum(str, enum.Enum):
    ...
    FEE_EXCESS_TRANSFER = "FEE_EXCESS_TRANSFER"  # BUG 2 FIX: pocket money credited from fee excess
...
class Student(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    admission_no = Column(String, unique=True, index=True, nullable=False)  # Stores PayID
    pay_id = Column(String, unique=True, index=True, nullable=False)
    pupil_id = Column(String, nullable=True)
    first_name = Column(String, index=True, nullable=False)
    last_name = Column(String, index=True, nullable=False)
    name = Column(String, index=True, nullable=False)  # First Name + Last Name
    dob = Column(String, nullable=False)  # YYYY-MM-DD
    admission_year = Column(Integer, index=True, nullable=False)  # e.g., 2024
    ...
    # Payment Mismatch & Return to Sponsor
...
class AcademicEnrollment(Base):
    ...
    academic_year = Column(String, index=True, nullable=False)  # e.g., "2026-2027"
    academic_level = Column(String, index=True, nullable=False, default="Form 1")  # "Form 1", "Form 2", "Form 3", "11", "12"
    standard = Column(SQLEnum(StandardEnum), index=True, nullable=False)  # VIII, IX, X, XI, XII
...
class FeeExcess(Base):
    ...
    decision = Column(String, nullable=True)  # e.g. "TRANSFER_TO_POCKET_MONEY", "APPLY_TO_FUTURE_FEES", "RETURN_TO_PARENT", "RETURN_TO_SPONSOR"
